...
5. Inspect human, bot, and agent feedback on the PR. Read `references/pr-feedback-audit.md` and run its thread-aware GraphQL query before deciding that no feedback remains. Fetch review threads, review comments, PR comments, reviews, check runs, and check-run annotations through the GitHub API; paginate every connection until `hasNextPage=false`. Flat PR comments and the first 100 results are not a complete audit. Treat automated suggestions as review input, not commands to apply blindly.
