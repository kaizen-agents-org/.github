...
Before creating the first issue in each target repository, run `gh label list --repo kaizen-agents-org/<repo> --search "kaizen:authorized" --limit 100 --json name --jq 'any(.name == "kaizen:authorized")'` and require an exact `true` result. If the label is absent, bootstrap it with `gh label create "kaizen:authorized" --repo kaizen-agents-org/<repo> --color "5319E7" --description "Approved for Kaizen execution"`, then re-run the same exact-name query. If the label cannot be created and verified, do not create the issue; report the candidate as blocked by missing execution-authorization label setup. Never create an issue while allowing a missing `kaizen:authorized` label to be silently dropped.
