...
require_text "${monitor}" 'only when the owner segment of `<repository>` is exactly `kaizen-agents-org`'
...
require_text "${readiness_creator}" 'the owner segment of `<repository>` is exactly `kaizen-agents-org`'
...
require_text "${contract_doc}" 'only for repositories whose owner is exactly `kaizen-agents-org`'
