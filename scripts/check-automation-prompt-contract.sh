...
require_text "${contract_doc}" 'The repository improvement scout also applies a generated-PR WIP guard per'
require_text "${contract_doc}" 'target repository. It creates no new issue when that repository has five or'
require_text "${contract_doc}" 'more open generated PRs. Branches or PR titles prefixed with `kaizen/`,'
require_text "${contract_doc}" '`codex/`, `claude/`, `[scout]`, `[monitor]`, or `kaizen:` count as generated'
require_text "${contract_doc}" 'guard is reached, the scout reports the eligible finding as skipped due to the'
require_text "${scout_doc}" 'the target repository has fewer than five open generated PRs.'
require_text "${scout_doc}" 'For the generated-PR WIP guard, branches or PR titles prefixed with `kaizen/`,'
require_text "${scout_doc}" '`codex/`, `claude/`, `[scout]`, `[monitor]`, or `kaizen:` count as generated'
require_text "${scout_doc}" 'repository and reports the eligible finding as skipped due to the WIP cap.'
