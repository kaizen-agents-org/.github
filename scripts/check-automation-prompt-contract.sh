...
require_text "${scout}" 'Before using any located checkout, read its `origin` URL'
require_text "${scout}" 'normalize supported HTTPS/SSH GitHub URL forms and casing'
require_text "${scout}" 'require it to match the target repository'\''s complete `kaizen-agents-org/<repo>` identity.'
require_text "${scout}" 'Never accept a directory based only on its expected local name.'
require_text "${scout}" 'use the target repository'\''s GitHub default-branch content instead.'
...
require_text "${scout}" 'Treat branches or PR titles prefixed with `kaizen/`, `codex/`, `claude/`, `agent/`, `[scout]`, `[monitor]`, or `kaizen:` as generated PRs unless evidence shows they are human-authored maintenance.'
...
require_text "${scout_doc}" 'normalizes the'
require_text "${scout_doc}" 'complete repository identity to match the intended `kaizen-agents-org`'
require_text "${scout_doc}" 'target repository'\''s GitHub default-branch content.'
...
require_text "${contract_doc}" 'Branches or PR titles prefixed with `kaizen/`, `codex/`, `claude/`, `agent/`, `[scout]`, `[monitor]`, or `kaizen:` count as generated.'
...
require_text "${repo_root}/docs/improvement-playbook.ja.md" '### A-3. WIP 制限を仕組みにする ⬜'
require_text "${repo_root}/docs/improvement-playbook.ja.md" '2. ⬜ 自動化: kaizen-loop の issue 選択時に対象リポジトリの open 生成 PR 数を数え'
require_text "${scout_doc}" 'for the fixed organization-wide scout, the target repository has fewer than'
require_text "${scout_doc}" "For the fixed organization-wide scout's generated-PR WIP guard, branches or PR titles prefixed with \`kaizen/\`, \`codex/\`, \`claude/\`, \`agent/\`, \`[scout]\`, \`[monitor]\`, or \`kaizen:\` count as generated."
