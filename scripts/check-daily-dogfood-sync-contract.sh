...
for repo in coderabbit kaizen-loop; do
  ...
  grep -q "^[[:space:]]*scheduler:" "${config}"
  grep -q "^[[:space:]]*jobs:" "${config}"
  grep -q "^[[:space:]]*maintenance:" "${config}"
  grep -q "^[[:space:]]*maintenance-followup:" "${config}"
  grep -q "^[[:space:]]*issue-watch:" "${config}"
