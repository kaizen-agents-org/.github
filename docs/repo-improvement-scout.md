...
The scout adds the `kaizen` label to created issues. It creates at most two issues per target repository per run and at most six issues total per run. Additional eligible findings stay in the report.
