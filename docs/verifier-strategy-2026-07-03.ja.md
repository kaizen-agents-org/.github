...
- **解決手段**: Step 0(TMPDIR 修正・語彙固定)→ Step 1(judge 純関数、設計書のコードを移植)→ Step 2(EVAL.md 準拠コーパス + ベースライン計測)→ Step 3(構造化出力で Claim 抽出)→ Step 4(レンズ + 反証、fpRate で効果を証明)→ Step 5(シャドーモードで本番採点)→ Step 6(cli/api プローブ)。

最初の一歩は明確: **今週、Step 0 と Step 1 に着手する。** どちらも LLM 不要・設計書に答えが書いてある作業で、失敗リスクがほぼない。
