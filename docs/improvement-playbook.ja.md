...
3. ⬜ 安定した checkout を列挙した `~/.kaizen/fleet.yml` をレビューし、operator が `kaizen fleet --manifest ~/.kaizen/fleet.yml --prune --verify` を実行して active fleet の verify が通ることを確認(readiness の「fleet refresh 未証明」ギャップを閉じる)。monitor automation 自身は `--dry-run` plan の確認までに留める。
