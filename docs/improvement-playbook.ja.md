...
- ✅ Issue 本文を untrusted input として扱う: builder プロンプトへは「参照データ」として明示的にラップし、指示と分離(kaizen-loop#198、PR#176 — .github#121 コメントでの報告に基づく。kaizen-loop リポジトリ未検証)。
...
| 2026-07-12 | .github#121 の追跡状態を更新。.github#119(本リポジトリで自己検証済み)完了を反映し、2026-07-08 レポートの G4 stale citation を訂正。kaizen-loop#198・verifier#94 の完了は .github#121 コメントの報告に基づくが、kaizen-loop/verifier リポジトリはこのワークツリーで検証できないため未検証と明記 | verifier#81/#97 と kaizen-loop#200 は未完。C-1/C-2、B-1/B-4 の出口条件は引き続き未達 |
