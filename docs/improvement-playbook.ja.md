...
現在 12 件(kaizen-loop 3 / builder-agent 5 / verifier 4)。以下の順でレビュー・マージする:

1. ⬜ **verifier #64**(tsx IPC path 失敗の回避)— 最優先。検証失敗 4/13 の主因 #48 を解消し、以後のすべての run の成功率を上げる。
2. ⬜ **verifier #65 → #66**(eval ケース追加 → 閾値ゲート)— BEHIND なので先に rebase。#66 は #65 の後。
3. ⬜ **builder-agent #70 → #71 → #72 → #73**(task understanding → provider evidence 成功 → 同ブロック時 → iteration artifacts)— 契約強化系。#72 は BLOCKED、A-2 の解消後にマージ。
4. ⬜ **kaizen-loop #153 → #154**(prompt を commit から除外 → payload parser 整合)。
5. ⬜ **dogfood sync 系(kaizen-loop #155 / builder-agent #74 / verifier #71)**— 上記のマージで陳腐化する可能性が高い。最後に確認し、daily sync が作り直すなら close でよい。

レビュー時の判断基準: 契約(schema/docs)とテストが揃っているか、diff が issue の意図の範囲内か。迷ったら `kaizen:needs-human` コメントを残して次へ(滞留させない)。

**完了判定**: `gh pr list -R kaizen-agents-org/{kaizen-loop,builder-agent,verifier}` がすべて 0 件、対応する issue が閉じている。

### A-2. BLOCKED の根本原因を解消する ⬜

builder-agent #72 #74、verifier #71 が checks 成功なのに `mergeStateStatus: BLOCKED`。06-29 readiness でも同事象が指摘済みで、branch protection と自動生成 PR の相性問題の疑いが強い。
...
**完了判定**: WIP 超過時に新規着手がスキップされるログが 1 回観測できる。scout の新カデンツが automations README と一致。
...
**完了判定**: 実運用レイアウトで生成された smoke artifact が 1 件以上存在し、#131 が閉じ、週次ジョブが登録済み。
...
3. ⬜ `kaizen fleet --root .. --owner kaizen-agents-org --repo .github --repo builder-agent --repo kaizen-loop --repo verifier --prune --verify` を実行し、全リポジトリの verify が通ることを確認(readiness の「fleet refresh 未証明」ギャップを閉じる)。
