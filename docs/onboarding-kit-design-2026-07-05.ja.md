...
位置づけ: [product-adoption-plan-2026-07-05.ja.md](./product-adoption-plan-2026-07-05.ja.md)(いつ・どの条件で載せるか)の姉妹編として、**どうやって載せるか**を設計する。Phase C-1(kaizen-loop#199、旧 kaizen-loop#173)の詳細設計を含む。
...
- 信頼できない起票者からの issue 受付(kaizen-loop#198/#200(旧 kaizen-loop#160/#174)完了まで。[adoption plan §4-3](./product-adoption-plan-2026-07-05.ja.md))
...
- **起動条件**: `kaizen:run` ラベルは書き込み権限者しか付与できないため、これ自体が execution authorization(kaizen-loop#200、旧 kaizen-loop#174)の実装になる。ラベル付与者の権限チェックを workflow 冒頭で再検証する(ラベルは剥がし忘れが起きるため、実行後に自動で剥がす)。
...
フロー図(六角形 = 人の承認ポイント、破線 = 再実行によるアップデート経路):

```mermaid
flowchart TD
    CMD["curl …/onboarding/onboard.sh を対象リポジトリ内で実行(冪等)"] --> S1
    S1["1. ツールチェーン install/update — versions.json と突合(§6.6)"] --> S2
    S2{{"2. スタック検出 → プロファイル提案 — 承認 1/3: verify コマンド(§6.2)"}} --> S3
    S3["3. kaizen init --profile — config + ラベル + issue テンプレ + skills vendor(§6.1/§6.8)"] --> S4
    S4{{"4. branch protection 適用 — 承認 2/3: 管理権限操作(§6.3)"}} --> S5
    S5["5. kaizen doctor --repair — プリフライト(既存コマンド)"] --> S6
    S6{{"6. kaizen smoke --yes — 承認 3/3: sandbox Issue→PR、artifact 保存(§6.5)"}} --> S7
    S7["7. check-onboarding-contract.sh — 契約充足の機械判定(§6.4)"] -- green --> DONE(["導入完了 — Issue→PR ループ稼働開始"])
    DONE -. "再実行 = アップデート(§7.1)— 差分だけ動く" .-> CMD
    classDef human fill:#F6EAD8,stroke:#B45309,color:#7C3A06;
    class S2,S4,S6 human;
```
...
- **外部起票者の受け入れは本キットのスコープ外**: kaizen-loop#198(untrusted input のデータブロック化、旧 kaizen-loop#160)と kaizen-loop#200(safety デフォルト監査、旧 kaizen-loop#174)の完了が前提。contract check に「kaizen-loop#198 完了前は外部公開リポジトリで `issues.label` 自動監視を有効化しない」ガードを入れる。
...
| 2 | GitHub Actions 型: reusable workflow + caller テンプレート | kaizen-loop#199(C-1、旧 kaizen-loop#173)、kaizen-loop#200(実行認可、旧 kaizen-loop#174)、リリースタグ導入 | Phase B 完了後 |
