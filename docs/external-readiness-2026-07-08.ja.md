...
回答: **万全とは言えない。** 導入の「入り口」(init / doctor / onboarding kit 設計)は形になりつつあるが、非 Node 実績、実行環境の可搬性、信頼境界の残件、判定品質の証拠が未完了である。プレイブックの区分で言えば、Phase B-4(汎化検証)と Phase C(外部ハーネス)の出口条件は未達。

> **2026-07-12 追記:** 本文は 2026-07-08 時点のスナップショットである。.github#119(A-5)完了は本リポジトリの `docs/metrics/` 実在で自己検証済み。kaizen-loop#198(G4 の構造的ラップ)・verifier#94(fixture の blocker)の完了は issue #121 のコメント(2026-07-11〜12 セッション記録)による報告であり、kaizen-loop/verifier リポジトリはこのワークツリーに存在しないため本セッションでは未検証。verifier#81 は残作業を verifier#97 が引き継ぐため open、G4 は kaizen-loop#200 の authorization / safety defaults 監査が残る。
...
| onboarding kit の設計と issue 化 | [onboarding-kit-design-2026-07-05.ja.md](./onboarding-kit-design-2026-07-05.ja.md)、.github#112、kaizen-loop#178(profile overlay)、kaizen-loop#179(非 Node stack 検出) |
...
現在 23 ケース(golden 12 + seeded 11)。目標は 50+ / 非 Node 10+。非 Node は現状 5 ケース。fixture 形式(実リポジトリ + 注入バグ)への移行は verifier#81 が担うが、verifier#94 の通り blocked 状態で、後続の verifier#82(LLM Stage 0)の受け入れ条件を塞いでいる。**「verifier が外部スタックでも正しく block/pass できる」ことの証拠が薄いまま**。
...
### G4. 信頼境界の残件(Playbook C-2 未完了)

2026-07-08 の調査で引用した `prompt.ts:24,95` の生補間は stale citation だったと issue #121 のコメント(2026-07-11〜12 セッション)で報告されている。報告によれば kaizen-loop PR#176(2026-07-05)で issue 本文・コメントは `taggedDataBlock()` によりデータブロック化され、タグ区切り文字も `escapeTagDelimiters()` でエスケープ済みであり、kaizen-loop#198 は close された。`src/goals/runner.ts` は goal-planner が生成したテキストの GitHub issue 向けデータシンクで、第三者由来入力のプロンプト補間ではない、との調査結果も報告されている。**これらの kaizen-loop リポジトリに関する主張は本セッションでは未検証**(kaizen-loop はこのワークツリーに存在しない)。残件は kaizen-loop#200 の execution authorization ラベル必須化と、`safety.envAllowlist` / protected paths の外部基準監査である。

### G5. 「同水準」を測るメトリクス基盤(Playbook A-5、2026-07-12 完了)

Phase C 出口条件は「外部プロダクトで人手修正なしマージ率が自組織と同水準」。2026-07-08 時点では北極星メトリクスの週次永続化が存在しなかったが、.github#119 の完了により `docs/metrics/<ISO週>.md` の契約と初回スナップショットが追加された。今後は週次スナップショットを継続し、外部適用の比較基準として使う。

## 3. アクション(issue マッピング)

| ギャップ | 対応 issue | 状態 |
| --- | --- | --- |
| G1 stack 検出 | kaizen-loop#179(非 Node stack 検出)、kaizen-loop#178(profile overlay)、.github#112(onboarding kit) | 既存 open |
| G1 完走実績 | **.github#120**: 非 Node リポジトリで Issue→PR→マージを 1 件完走(B-4) | 新規作成済み |
| G2 コーパス | verifier#94(blocker 解消済み)/ verifier#81(fixture corpus、open)→ **verifier#97**: 50+/非 Node 10+ への拡充 | 進行中 |
| G3 可搬性 | **kaizen-loop#199**: C-1 GitHub Actions reusable workflow(Actions secrets 認証含む) | 新規作成済み |
| G4 信頼境界 | kaizen-loop#198: 構造的ラップ(完了) + **kaizen-loop#200**: authorization ラベル必須化・safety デフォルト外部基準監査 | 一部完了 |
| G5 メトリクス | **.github#119**: 週次メトリクスを `docs/metrics/` に PR で永続化(A-5 完了) | 完了 |
