...
authority; the JSON above is a shape example. That set passed clean-install and
doctor verification. The preceding set with `kaizen-loop v0.1.6`
completed the issue-to-merge run on a repository outside this organization; see
[`first-external-run-2026-08-12.md`](./first-external-run-2026-08-12.md).

Configurations generated by `kaizen init` under `v0.1.4` and later set
`verifier.expectedRef` to a `refs/tags/` value, which the `v0.1.3` schema
rejects. **Adopters cannot downgrade below `v0.1.4` after running `init`.**
